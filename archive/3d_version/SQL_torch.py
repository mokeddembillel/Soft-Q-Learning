...
############################ LEARN ###############################    
    def learn(self, steps):
        ...
        if steps%5 == 0:
            ...
            updated_actions = action_svgd
            with T.no_grad():
                fixed_actions= action_svgd
                
            #fixed_actions.requires_grad = False            
            
            #print('action_svgd', action_svgd)
            #print(state)
            svgd_Q_soft = self.get_Q_value(state, fixed_actions, training=True, particles=True) # (bs, np, 1)
            ...
            kappa, grad_kappa = self.rbf_kernel(input_1=fixed_actions, input_2=updated_actions) # (bs, np, ad)
            ...
            T.autograd.backward(-updated_actions, grad_tensors=svgd)
